...
const STOP_KIND = {
  ...
  session: {
    ...
    ask: {title: "일하는 창에 중단하라고 전할까요?",
      desc: "지금 하던 말은 그 창이 마치는 데까지 이어질 수 있어 곧바로 멎지"
        ...
        + " 무인 작업은 이 요청을 다시 맡지 않습니다. " + STOP_ASK_TAIL,
      ok: "중단하라고 전하기"}},
  agent: {
  ...
  /* idle 은 ux-writer 가 처음부터 "줄을 세우지 말라"고 했다("도는 것이 없으니
     ...
     제자리로 돌린다. */
  idle: {
    who: "지금 이 요청을 담당하는 것이 없습니다 — 맡은 창도, 일손도, 무인 작업도 없습니다",
    ...
    /* 단추(「자동 이어받기 끄기」)가 축을, 이 글이 사실을 말한다
       ...
       사고가 조건절로 옮겨 앉는다. 결과는 「무인 작업이 뜨지 않습니다」로
       좁혀 말한다: 「아무것도 뜨지 않습니다」는 내가 여는 창까지 삼켜
       거짓이 된다. */
    tip: "지금 이 요청에 담당이 없습니다 — 이대로 두면 잠시 뒤 무인 작업이"
      + " 떠서 이어받습니다. 꺼 두면 「▶ 이어가기」를 누를 때까지 무인 작업이"
      + " 뜨지 않습니다."},
};
...
/* 예고 줄 — 단추가 서는 곳에 **함께** 서는 사실 한 줄 (REQ-20260901-005
   designer 1안). 개념(담당이 없으면 무인 작업이 떠서 이어받는다)을 가르칠
...
function holdForecastHTML(r){
  ...
  return `<div class="rvpt fore" id="pol-fore">`
    ...
    + `담당이 없으면 무인 작업이 떠서 이 요청을 이어받습니다</div>`;
}
