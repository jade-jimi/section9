...
/* 글자로 적힌 경로 → 서버가 받는 리포 상대경로. 못 만들면 "" (=링크 안 세움). */
function codeRel(raw){
  let q = String(raw || "").replace(/\\/g, "/").trim();
  if (/^~?\//.test(q)){
    /* 절대경로는 **워크스페이스 이름 뒤부터**만 읽는다. 서버는 리포 상대경로만
       받는데 화면은 리포 뿌리를 모른다 — 그리고 알려 달라고 하지 않는다(경로도
       값이다, REQ-20260828-012). 이미 손에 있는 whoami.workspace 로 자르고,
       그 이름이 없으면 링크를 세우지 않는다: 짐작으로 잘라 **남의 리포 경로를
       이 리포의 파일로 열어 보이는 것**이 못 여는 것보다 나쁘다. */
...
/* 절대경로(~/ 포함)와 리포 상대경로 두 규칙. 상대경로 쪽은 **허용 뿌리로
