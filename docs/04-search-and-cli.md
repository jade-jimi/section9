...
- `--sweep` — 주인 없는 캡처 잔여물(표식 `s9shot-`/`cdp-prof-` 을 가진 오래된
  브라우저·프로필 디렉토리)만 조용히 회수한다. 캡처가 자기 것을 지우는 경로는
  세션이 중간에 죽으면 돌지 않고, 그렇게 남은 브라우저는 리눅스 `ps` 에도
  잡히지 않는다(윈도우 쪽에만 남는다) — 2026-08-25 사고의 씨앗이 그것이었다.
  나이가 유일한 기준이라 진행 중인 캡처는 건드리지 않는다. `s9 shot` 이 캡처
  직전에(3분에 한 번으로 묶어) 부르고, serve 의 포트 감시도 임계에서 부른다.
...
진단은 사람이 불러야 돈다 — 그런데 저 사고는 사람이 "인터넷이 안 된다"를
겪고 나서야 알았다. 그래서 `s9 serve` 가 5분마다 호스트 동적 포트 소진도를
재고 임계마다 스스로 손을 쓴다.

| 소진도 | 행동 |
|---|---|
| 60% 미만 | 아무것도 하지 않는다 |
| 60% 이상 | `--sweep` — 우리가 흘린 잔여물만 회수(비파괴) |
| 90% 이상 | `--recover` — 고갈되기 전에 점유자 회수(중계는 자동 재기동) |

기록은 `state/port-guard.log`. 끄기: `S9_PORT_GUARD=off`.
90%에서 자동 회수하는 이유는 100%가 곧 **OS 전체 아웃바운드 단절**이고, 그
지점에서는 사람도 브라우저도 손쓸 수단이 없기 때문이다 — 회복 수단이 남아
있는 동안 쓴다.
