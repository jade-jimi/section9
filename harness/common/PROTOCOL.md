...
모든 응답은 **현재 시각과 응답 주체 이름** 헤딩 한 줄로 시작한다 — 대괄호 안에
한 덩어리로 묶고, 백틱으로 감싼다:

```
# `[2026-08-26 22:05:40 KST - lead]`
```

백틱은 장식이 아니다: 헤딩(#)만으로는 강조가 한 겹이라 본문 제목과 섞인다.
인라인 코드로 감싸면 렌더러가 다른 색을 입혀 시각·주체가 한눈에 갈린다.
