...
class TestTermConn(unittest.TestCase):
    ...
    def test_conn_setter_exists_with_states(self):
    ...
    def test_conn_element_in_status_strip(self):
        self.assertRegex(
            self.html, r'id="cc-conn"[^>]*hidden',
            "상태줄에 cc-conn 표시 요소(기본 hidden)가 없다")

    # S1. SSE 단절 → '재접속 중' 경고
    def test_sse_error_marks_retry(self):
    ...
    # S4. SSE 복구 → 표시 소거
    def test_sse_recovery_clears(self):
        body = _fn_body(self.html, "termConnectSSE")
        self.assertIsNotNone(body)
        self.assertRegex(
    ...
    def test_poll_fallback_states(self):
    ...
    def test_meta_reflects_conn(self):
    ...
    def test_attach_resets_conn(self):
