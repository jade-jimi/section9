...
class StateTruth(unittest.TestCase):
    ...
    def test_n1_post_commit_installed(self):
    ...
    def test_n2_commit_recorded(self):
        rid = self.mk("커밋될 것")
    ...
    def test_b1_done_doc_skipped(self):
    ...
    def test_n3_idle_claim_expires(self):
    ...
    def test_b2_moved_after_claim_is_alive(self):
    ...
    # B3. 방금 잡은 것은 놓지 않는다
    def test_b3_fresh_claim_kept(self):
        rid = self.mk("방금 잡은 것")
        b = self.binding()
        b["active_reqs"] = [rid]
    ...
    def test_f2_unknown_claim_time_kept(self):
        rid = self.mk("옛 바인딩")
        b = {"active_reqs": [rid]}
        self.assertFalse(self.m.claim_dead(b, rid))

    # N4. 클레임하면 시각이 남는다
    def test_n4_claim_stamped(self):
        rid = self.mk("클레임할 것")
    ...
    def test_n5_loose_names_the_fix(self):
        rid = self.mk("커밋만 된 것")
