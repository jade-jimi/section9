...
class TestS9Sync(unittest.TestCase):
    ...
    def test_y7_staged_code_never_rides_along(self):
        """admin 이 WIP 코드를 stage 해둔 채 문서를 만들면, 미완성 코드가
        origin 으로 나가서는 안 된다. sync 는 데이터 경로만 담는다."""
    ...
    def test_y9_assets_ride_the_same_cycle(self):
        """첨부를 문서 옆으로 옮기는 일은 `s9 new` 가 끝난 뒤에 일어난다.
        옮긴 쪽에서 동기화를 돌리지 않으면 첨부만 로컬에 남고, 후속 이벤트가
        없으면 영영 나가지 않는다 — 아무도 모르는 데이터 미동기화다."""
    ...
    def test_y8_guarded_path_does_not_halt_sync(self):
        """non-admin 이 보호경로를 stage 하면 pre-commit(s9-guard)이 커밋을
        거부한다. 종전엔 그 거부가 문서 커밋까지 통째로 막았고, s9 는 rc=0 으로
        성공을 보고해 아무도 모르게 데이터가 안 나갔다. 커밋 범위를 데이터로
        한정하면 훅을 우회하지 않고도 이 정지가 사라진다."""
