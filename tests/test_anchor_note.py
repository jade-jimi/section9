...
class AnchorNote(unittest.TestCase):
    ...
    def test_it_sends_doc_and_anchor(self):
        ...
        **계약을 다시 썼다** (REQ-20260828-006): 보내는 자리가 `/api/chat` 에서
        `/api/note` 로 옮겨졌고, 세션을 요구하던 문지기는 사라졌다. 앞선 계약은
        "세션이 없으면 말없이 삼키지 말 것"을 요구했는데, 그 요구 자체가 틀린
        전제 위에 있었다 — 메모에는 세션이 필요 없다.
        """
    ...
    def test_pointing_at_a_span_does_not_change_the_screen(self):
        """짚는 손잡이는 **화면을 옮기지 않는다** (REQ-20260827-072 반려).

        사용자가 본 것: 메모의 "⌖ 그 구간" 을 누르는 순간 Board 로 튀었다.
        원인은 손잡이가 `<a href="#anq0">` 였다는 것 하나다 — 이 화면에서 해시는
        문서 안의 자리가 아니라 **라우트**(`#board`·`#docs/REQ-…`)라서, 조각
        링크를 눌러도 popstate 가 날아가고 라우터가 "anq0" 을 모르는 이름이라
        판정해 Board 로 떨어뜨린다.

        그래서 못을 둘 박는다: ① anchorMark 가 href 를 다시 만들지 않을 것,
        ② 짚는 두 방향이 **같은 재료**일 것. 이번 결함의 전부가 "한쪽만 href
        였다" 이므로, 한쪽만 고치면 반쪽만 고친 것이 된다.
        """
        fn = self._fn("anchorMark")
    ...
    def test_it_finds_a_span_that_crosses_bold_and_code(self):
        """**글자 마디 하나 안에서만 찾으면 안 된다** (REQ-20260827-072 2차 반려).

        사용자: "이 문서 자체에 새로 메모를 만들어봤는데, 기능이 어떻게
        동작했는지 파악도 안되고." 사용자가 고른 구간은 굵은 글씨와 `코드` 를
        가로지르는 한 문장이었고, 마크다운은 바로 그 자리에서 문장을 여러
        조각으로 쪼갠다(`<strong>`·`<code>`·문서 링크). 조각 하나 안만 뒤지던
        옛 코드는 **문서가 그대로인데도** "문서가 바뀌어 이 구간을 찾지
        못했습니다"라고 답했다 — 방금 고른 그 글에 대고.

        그러니 못은 둘이다: ① 본문을 한 줄로 이어 놓고 찾을 것, ② 찾은 구간을
        마디별로 잘라 각각 감쌀 것(Range 는 요소 경계를 가로질러 감쌀 수 없다).
        """
    ...
    def test_arriving_is_visible_even_without_scrolling(self):
        """**움직임을 결과의 증거로 삼지 않는다** (REQ-20260827-072 2차 반려).

        사용자: "이전에 등록했던 메모도 '그 구간' 이라는 링크를 클릭해봤을 때
        이동이 되지 않는다." 실제로는 눌리고 있었다 — 다만 메모와 그 구간이
        이미 같은 화면 안에 있으면 `scrollIntoView` 가 한 픽셀도 움직이지
        않는다. 기계는 제 할 일을 했는데 사람 눈에는 아무 일도 없었다.

        그래서 도착한 자리 자체를 잠깐 밝힌다 — 스크롤이 필요하든 아니든.
        색면으로 칠하지 않는다: 재료는 선 하나와 시간뿐이고 흔적을 남기지 않는다.
        """
        go = self._nocomment(self._fn("anchorGo"))
    ...
    def test_it_shows_the_memo_it_just_left(self):
        """**남긴 것을 보여 준다** (REQ-20260827-072 2차 반려).

        사용자: "이 문서 자체에 새로 메모를 만들어봤는데, 기능이 어떻게
        동작했는지 파악도 안되고." 전에는 "문서를 다시 열면 그 구간 옆에서
        읽힙니다"라고 **말만** 하고 화면은 그대로였다 — 자기가 한 일의 결과를
        보려면 사람이 스스로 새로 고쳐야 했다. 무슨 일이 일어났는지 알 길이
        없는 기능은, 되든 안 되든 사용자에게는 같은 것이다.
        """
        fn = self._fn("anchorSend")
        code = self._nocomment(fn)
    ...
    def test_the_diagnostic_is_readable_from_a_capture(self):
        """**확인한 사람만 아는 확인은 확인이 아니다.**

        둘을 못으로 박는다.

        ① 결과를 화면에 적을 것. 이 저장소에서 화면을 확인하는 손은
           `s9 shot`(스크린샷)이고 제목 표시줄은 스크린샷에 없다 — 결과를
           `document.title` 에만 찍으면 진단을 돌린 사람만 알고, 캡처를 문서에
           붙여 놓아도 다음 사람은 그 그림에서 아무것도 읽지 못한다.
        ② 손잡이가 생길 때까지 기다렸다 누를 것. 고정 시간 뒤에 누르면, 문서
           렌더가 그보다 늦은 기계에서는 아무것도 누르지 않은 채 지나간다.
           실제로 그 상태를 만났다 — '눌러 봤다'가 기계 사정에 따라 조용히
           헛것이 되는 확인은 회귀를 못 잡는다.
        """
    ...
    def test_the_diagnostic_can_press_the_last_handle(self):
        """사용자가 누른 것은 **마지막에 생성된** '그 구간' 이었다.

        문서에 앵커 메모가 둘 이상이면 손잡이도 둘 이상이고, 반려는 그중
        가장 나중에 달린 것에서 났다("마지막에 생성된 '그 구간' 을 클릭하는
        순간, 보드 화면으로 이동한다"). 첫 번째만 눌러 보는 진단으로는 그
        자리를 영영 지나친다 — 다음 사람이 "눌러 보고 확인했다"고 말할 때
        그 말이 **사용자가 누른 그 자리**를 뜻하도록 못을 박는다.
        """
        diag = self._fn("anchorDiag")
