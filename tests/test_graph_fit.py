...
class GraphFit(unittest.TestCase):
    ...
    def test_zoom_anchor_sits_between_cursor_and_centre(self):
        """커서 기준 그대로면 굴릴 때마다 화면이 대각선으로 밀린다."""
        w = self._wheel()
    ...
    def test_zoom_in_and_out_get_the_same_treatment(self):
        """한쪽만 고치면 다른 쪽이 그대로 남는다 (2차 반려 ㉣)."""
        w = self._wheel()
    ...
    def test_the_leash_keeps_the_screen_from_emptying(self):
        """화면보다 작으면 가운데로, 아직 넘치면 가장자리의 빈 띠만큼만."""
    ...
    def test_panel_height_is_measured_not_assumed(self):
        """고정 계산(innerHeight - N)은 머리글·범례가 한 줄 늘면 그만큼 어긋난다.

        판 바닥이 화면 밖으로 내려가면 판 안쪽에 붙은 손잡이가 잘린다 — 사용자가
        본 "전체 보기 버튼이 화면 하단에 잘려서 표시된다"가 이것이다.
        바로 옆 syncRailH() 는 같은 이유로 이미 잰 값을 쓰고 있었다.
        """
    ...
    def test_fit_verifies_its_own_iteration_before_moving(self):
        """되풀이가 수렴하지 않으면 배율이 하한까지 떨어져 노드를 화면 밖으로 민다.

        깊이 가중이 이동량에도 곱해져(화면 폭 ≈ k·월드폭 + 0.49·이동량) 무리가
        좁을 때 되먹임이 생긴다 — "버튼을 누르면 노드가 전부 사라진다"가 이것이다.
        그러니 옮기기 전에 재 보고, 안 보이면 안전한 기본값을 쓴다.
        """
        fn = self._fn("gFitTf")
    ...
    def test_physics_can_be_settled_by_hand(self):
        """헤드리스 캡처는 프레임이 몇 장만 돌아 처음 놓인 고리 배치 그대로 남는다.

        그 배치는 넓게 퍼져 있어 전체 보기가 늘 잘 되는 것처럼 보인다. 사람이 보는
        자리 잡힌 그래프는 가운데로 뭉쳐 있고 거기서 이번 결함이 났다 — 그래서
        3차까지의 검증이 사용자가 보는 것을 재지 못했다.
        """
    ...
    def test_before_and_after_can_be_put_on_the_same_ruler(self):
        """"뭐가 달라졌는지 모르겠다"에 그림 두 장으로 답할 수 있어야 한다."""
        w = self._wheel()
    ...
    def test_depth_does_not_shear_the_cluster_along_the_pan(self):
        """사용자: "멀리서 보면 구 모양이어야 할 것 같은데 ↖→↘ 로 찌그러졌다."

        깊이 시차를 **팬 오프셋에 곱하면**(n.x*k + tx*pf) 가만히 있을 때도 노드마다
        (pf−1)·(tx,ty) 만큼 밀린다 — 시차가 아니라 팬 방향으로의 전단이다. 무리는
        등방적으로 놓이는데(초기 배치는 원, 중심으로 당기는 힘도 등방) 화면에서만
        대각선으로 눌린다. 실측: 전체 보기 직후 긴 축/짧은 축 1.24, 기운 각도 20°
        (그때 팬은 459,209 — 각도 24° 다. 팬 방향과 같다).

        카메라를 화면 가운데에 두고 **거리에** 깊이 배율을 먹이면(원근투영이 실제로
        하는 계산) 가만히 있을 때는 등방 확대라 원이 원으로 남고, 팬 할 때는 깊이마다
        다르게 흘러 시차는 그대로 산다. 같은 자리에서 1.04 가 된다.
        """
