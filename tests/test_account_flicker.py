...
class TheAccountList(unittest.TestCase):
    ...
    def test_the_window_asks_through_the_retrying_path(self):
    ...
    def test_the_retry_helper_exists_and_loops(self):
    ...
    def test_a_good_answer_ends_it(self):
        fn = self._fn("ccFetchTry")
    ...
    def test_the_backoff_has_jitter(self):
        fn = self._fn("ccFetchTry")
    ...
    def test_the_lost_window_offers_a_way_out(self):
    ...
    def test_the_lost_window_does_not_offer_adding(self):
        """서버가 안 답하는 판에서 `＋ 계정 추가` 는 눌러도 안 된다 —
        되지 않을 일을 권하면 그 창은 두 번 거짓말을 한다."""
        fn = self._fn("acctFoot")
    ...
    def test_no_poll_redraws_the_open_window(self):
        """이 건의 이름이 「깜빡임」인 이상, 창을 다시 그리는 손이 어느 폴에도
        없다는 것까지가 계약이다. renderDocs·renderProjects 가 열린 창을
        덮어쓴 실사고 계보(docFresh·prjEditing 가드)가 이 저장소에 이미 있다."""
    ...
    def test_the_window_is_painted_in_one_place(self):
    ...
    def test_the_failure_can_be_staged_without_hands(self):
        fn = self._fn("ccFetchTry")
