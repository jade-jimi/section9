...
class ShortRefRule(unittest.TestCase):
    ...
    def test_the_answer_is_the_newest_one_that_existed_when_the_line_was_written(self):
        rows = self.rows(
    ...
    def test_a_newer_document_does_not_change_what_an_old_line_meant(self):
        """기록이 나중에 다른 것을 가리키면, 어제 쓴 노트가 오늘 거짓말을 한다."""
    ...
    def test_the_number_is_compared_as_a_number_not_as_text(self):
        """`REQ-28` 과 `REQ-028` 은 같은 말이다."""
    ...
    def test_a_different_kind_is_a_different_number(self):
    ...
    def test_a_number_that_did_not_exist_yet_is_not_guessed(self):
        """지어낸 링크는 없느니만 못하다 — 그게 "최근 우선"의 가장 나쁜 결말이다."""
        rows = self.rows(("REQ-20260827-041-62x6", "2026-08-27T10:00:00+09:00"))
        self.assertIsNone(self.m.resolve_short(
            "REQ", "041", "2026-08-21T18:02:06+09:00", rows))

    def test_without_a_write_time_nothing_is_resolved(self):
    ...
    def test_only_a_kind_prefix_and_a_hyphen_makes_a_short_reference(self):
        """일반 낱말·판 번호·`#028` 에 번지면 안 된다."""
    ...
    def test_the_screen_and_the_server_share_one_contract_table(self):
        """표는 화면에 적혀 있고, 서버가 그 표를 통과해야 한다.

        규칙이 두 벌인 것 자체가 위험이라, 한쪽이 어긋나면 반드시 여기가
        빨개진다. 표의 모든 줄은 **작성시각 이전**의 문서로 풀리므로 새 문서가
        생겨도 답이 바뀌지 않는다 — 그 고정성 자체가 검사 대상이다.
        """
    ...
    def test_the_screen_resolves_by_write_time_too(self):
    ...
    def test_a_guess_cannot_be_written_into_a_document(self):
        """이어 말하기는 그 문서에 영구 기록을 남긴다 — 짐작에는 주지 않는다."""
        code = re.sub(r"/\*[\s\S]*?\*/", "", self.src)
    ...
    def test_a_guess_link_does_not_look_like_a_certain_one(self):
        """확실한 링크와 같은 얼굴이면 사람은 그것이 짐작인 줄 모른다."""
    ...
    def test_a_document_that_does_not_exist_stays_plain_text(self):
        code = re.sub(r"/\*[\s\S]*?\*/", "", self.src)
    ...
    def test_both_places_ask_the_same_function(self):
        """판정이 두 곳으로 갈리면 같은 글자가 터미널과 본문에서 다르게 보인다."""
        code = re.sub(r"/\*[\s\S]*?\*/", "", self.src)
...
class ShortRefCorpus(unittest.TestCase):
    ...
    def test_the_render_time_rule_would_have_been_wrong_most_of_the_time(self):
        """이 수치가 작으면 기준시각을 옮긴 이 작업이 헛수고였다는 뜻이다."""
        latest = {}
        for r in self.rows:
