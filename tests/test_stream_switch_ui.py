...
class StreamSwitchUI(unittest.TestCase):
    ...
    def test_the_switch_exists_in_my_account(self):
        """CLI 에만 있던 스위치가 화면에 있어야 한다."""
        blk = self._block()
    ...
    def test_it_saves_the_moment_you_change_it(self):
        """저장 버튼을 따로 두면 "켰는데 왜 안 켜지지"가 생긴다."""
    ...
    def test_keep_days_recedes_when_recording_is_off(self):
        """칸만 잠그면 옆의 설명이 아직 쓸 수 있는 것처럼 남는다."""
    ...
    def test_the_words_explain_the_edges(self):
        """빈 값과 0 의 뜻을 미리 말하고, 잘못 적은 것을 비난하지 않는다."""
        blk = self._block()
    ...
    def test_it_can_be_flipped_without_hands_and_restored(self):
        """select 에 값이 들어 있는 것만으로 "끄고 켤 수 있다"를 말할 수 없다."""
