...
class TestProjectAgents(unittest.TestCase):
    ...
    def test_a1_a4_sync_and_namespace(self):
    ...
    def test_a2_idempotent_update(self):
    ...
    def test_a3_prune_managed_only(self):
    ...
    #     프로젝트 무관 REQ는 미주입
    def test_a6_worker_preamble(self):
        ...
        try:
            ...
            self.assertIn("프로젝트 에이전트 규정(alpha/agents/worker.md)", pre)
            ...
            prompt = calls[0][2]
            self.assertTrue(prompt.startswith("<<프로젝트 에이전트 규정"),
                            prompt[:120])
    ...
    def test_a7_scaffold_and_guide(self):
