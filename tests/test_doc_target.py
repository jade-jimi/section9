...
class DocTarget(unittest.TestCase):
    ...
    def test_a_card_can_be_picked(self):
        """카드에 집는 손잡이가 있고, 카드 열기보다 먼저 잡힌다."""
    ...
    def test_the_viewer_can_pick_too(self):
        """읽고 있는 자리가 곧 집는 자리다 — 목록으로 돌아가지 않는다."""
        self.assertIn('data-pick="${esc(m.id)}"', self.src,
                      "문서 뷰어에서 집을 수 없다")

    # ---------- ② 입력창에서 보인다 ----------

    def test_the_input_says_where_it_goes(self):
        """집혔다는 것이 입력창 바로 위에서 보인다 — 지목과 같은 자리, 같은 어휘."""
    ...
    def test_release_is_one_gesture(self):
    ...
    def test_nothing_changes_when_nothing_is_picked(self):
        """대상이 없으면 줄 자체가 없다."""
        fn = self._fn("termTargetRender")
    ...
    def test_agent_and_doc_are_exclusive(self):
        """메시지는 에이전트에게 가거나 문서 노트로 들어간다 — 둘 다일 수 없다."""
    ...
    def test_a_finished_request_says_so_before_sending(self):
        """보내고 나서 알려 주면 되돌릴 수 없다 — 집은 순간에 말한다."""
        fn = self._fn("termTargetRender")
