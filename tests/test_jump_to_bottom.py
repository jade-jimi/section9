...
class JumpToBottom(unittest.TestCase):
    ...
    def test_hidden_while_stuck_to_the_bottom(self):
        """바닥에 붙어 있으면 손잡이도 안 읽은 셈도 없다."""
    ...
    def test_it_appears_exactly_when_auto_follow_stops(self):
        """손잡이가 뜨는 순간 = 자동 따라가기가 멎는 순간."""
    ...
    def test_it_counts_what_piled_up_while_reading(self):
        """위로 읽는 동안 온 줄만 센다 — 화면에 안 그려진 이벤트는 안 센다."""
        body = self._fn("termAppendBatch")
    ...
    def test_switching_pane_restarts_the_count(self):
        """에이전트 타임라인과 main 은 다른 판이다 — 셈을 물려주지 않는다."""
    ...
    def test_two_states_differ_by_weight_not_hue(self):
        """안 읽은 쪽은 인버스 — 색면 하이라이트가 아니다."""
        css = self._css()
    ...
    def test_the_key_it_advertises_actually_works(self):
        """라벨에 Ctrl+End 를 적었으면 Ctrl+End 가 들어야 한다."""
    ...
    def test_it_is_reachable_without_a_mouse(self):
        """손잡이는 진짜 <button> 이다 — 흉내 낸 자리는 키보드가 못 누른다."""
        mk = self._markup()
    ...
    def test_it_can_be_opened_without_hands(self):
        """헤드리스로 직접 보고 고칠 길 (?usagecard·?dlg 과 동형)."""
