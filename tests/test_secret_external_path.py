...
덤으로 지키는 것: **리포 안 경로는 뜻이 없다**(리포는 공유·공개된다), 그리고 **s9 는
바깥 폴더를 읽기만 한다** — `s9 secret set` 도 화면의 ＋넣기도 언제나 저장소 안에
쓴다. 이 사실이 화면 문구에서 오해되면 사람이 값을 엉뚱한 데 넣는다.
...
class ExternalPathUI(unittest.TestCase):
    ...
    # B3. **읽기만 한다** — 여기 넣으면 되는 줄 알면 값이 엉뚱한 데로 간다
    def test_b3_it_says_s9_only_reads_that_folder(self):
        mine = self._mine()
        note = mine[mine.index("바깥 폴더 —"):mine.index('id="sec-ext"')]
        self.assertIn("읽기만 합니다", note, "읽기 전용이라는 것을 말하지 않는다")
        self.assertIn("저장소 안에 씁니다", note,
                      "＋넣기가 어디에 쓰는지 말하지 않는다")
        self.assertIn("파일 이름이 곧 키 이름", note,
                      "그 폴더에 어떻게 넣는지 말하지 않는다")
