...
class TheShard(unittest.TestCase):
    ...
    def test_p1_greedy_by_size_covers_every_file_once(self):
    ...
    def test_p2_serial_files_stay_out_of_shards(self):
    ...
    def test_p3_children_are_nested(self):
        src = open(RUNNER, encoding="utf-8").read()
        self.assertIn('"S9_TESTS_NESTED": "1"', src,
                      "자식이 nested 가 아니면 reap·잡파일을 부모와 다툰다")

    def test_p4_failed_shard_replays_its_output(self):
        src = open(RUNNER, encoding="utf-8").read()
    ...
    def test_p5_jobs_gate_only_above_one(self):
        src = open(RUNNER, encoding="utf-8").read()
        self.assertIn("if jobs > 1 and not nested:", src,
                      "--jobs 1 이 직렬 현행 경로를 벗어난다")

    def test_p1b_live_two_shards_run_green(self):
