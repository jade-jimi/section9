...
class RestartReach(unittest.TestCase):
    ...
    def test_it_does_not_need_the_terminal_pane(self):
    ...
    def test_the_session_id_comes_from_the_answer_it_already_has(self):
        """목록을 받아 온 그 답이 세션 id 를 이미 들고 있다 — 없는 것을
        TERM 에서 꺼내려다 조용히 실패하지 않는다."""
        i = self.src.find("async function claudeAccountSwitch(")
        blk = self.src[i:i + 2000]
    ...
    def test_nothing_returns_silently(self):
        """예전엔 `if (TERM !== T || !out || !w) return;` 이 답의 전부였다."""
    ...
    def test_every_branch_says_something(self):
        """수락·거부·손이 필요함 — 셋 다 사람에게 닿아야 한다."""
        tell = self._code("restartTell")
    ...
    def test_it_is_a_chip_not_a_new_row(self):
        """REQ-20260827-017: 줄은 사람의 손을 요구하는 사실에만 준다.
        다시 시작하는 중인 것은 읽고 나서 할 일이 없다 — 줄을 주면 헤더가
        그만큼 눌리고, 보드가 밀린다."""
    ...
    def test_the_chip_does_not_reread_itself_every_second(self):
        """`#sv-chip` 은 aria-live 영역이다 — 글자가 매초 바뀌면 화면 낭독이
        되풀이된다. 흐르는 시간은 낱말이 아니라 도는 마크가 말한다."""
    ...
    def test_the_good_news_leaves_by_itself(self):
        """잘된 일에 닫는 손을 요구하지 않는다. 못 바꾼 것은 손이 필요한
        사실이라 스스로 사라지지 않는다."""
        fn = self._code("restartChip")
    ...
    def test_it_asks_before_it_stops(self):
        """말없이 끊지 않는다 — 하던 일을 끊는 것은 사람이 알고 눌러야 한다."""
        tell = self._code("restartTell")
    ...
    def test_the_stop_uses_the_road_that_exists(self):
        """중단 신호를 두 벌로 만들면 한 벌만 고쳐진다 — Esc 가 쓰는 그 길이다."""
        fn = self._code("restartAfterStop")
        self.assertIn('"/api/chat"', fn, "중단을 청하지 않는다")
        self.assertRegex(fn, r'kind: "interrupt"', "다른 길로 멈추려 한다")

    def test_it_speaks_while_it_waits(self):
        """멈춘 뒤 유휴가 되기까지 시간이 걸린다 — 그 사이가 침묵이면 또
        '아무런 반응이 없다'가 된다."""
        fn = self._code("restartAfterStop")
    ...
    def test_it_retries_only_while_busy(self):
        """일하는 중이 아닌 다른 사유로 거부됐으면 계속 두드리지 않는다."""
        fn = self._code("restartAfterStop")
    ...
    def test_the_server_guard_is_untouched(self):
        """가드를 없애면 말없이 끊기는 일이 생긴다 — 화면이 먼저 멈추고 나서
        다시 청하는 순서다. 화면에서 고쳤다고 서버를 손대지 않았는지 본다.

        판정의 **출처**는 옮겼다 (REQ-20260901-011): 손으로 파싱하던 자리가
        공유 판독기(`transcript_read`)로 갔다 — 한도로 굳은 턴을 「진행 중」
        으로 읽어 전환이 통째로 막히던 결함의 근원이다. 가드 자체는 그대로다:
        **진짜 도는 턴은 여전히 사람이 명시하지 않는 한 끊기지 않는다.**"""
    ...
    def test_the_model_dialog_takes_the_same_road(self):
    ...
    def test_the_dialogs_no_longer_promise_idle_only(self):
        """`세션이 쉬고 있을 때만 됩니다` 는 이제 거짓이다."""
    ...
    def test_it_can_be_seen_without_hands(self):
        """이 화면들은 그 순간이 와야만 보인다 — 세션이 일하는 중이어야 하고,
        서버가 거부해야 한다. 캡처로 재현할 수 없다."""
